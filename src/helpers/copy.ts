...
export const getSession = (player: string, login: string) => new Promise((res, rej) => {
	...
	// } else {
		...
		shell.exec(`scp -r root@${serverIp}:"/root/puppet/${remoteFolder}/Default/*.*" /root/puppet/puppet/${folder}/Default/`, { silent: false })
		shell.exec(`scp -r root@${serverIp}:"/root/puppet/${remoteFolder}/*.*" /root/puppet/puppet/${folder}/`, { silent: false })
	// }
...
export const copyBack = (player: string, login: string) => new Promise((res, rej) => {
	...
	// } else {
		...
		shell.exec(`scp -r /root/puppet/puppet/${folder}/Default/*.* root@${serverIp}:"/root/puppet/${remoteFolder}/Default/"`, { silent: false })
		shell.exec(`scp -r /root/puppet/puppet/${folder}/*.* root@${serverIp}:"/root/puppet/${remoteFolder}/"`, { silent: false })
	// }
